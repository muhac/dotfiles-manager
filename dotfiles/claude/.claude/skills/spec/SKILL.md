...
**README.md** — shared context loaded by every `/impl` session. Use the [readme template](templates/readme.md). Sections marked `<!-- workspace only -->` are included only for multi-repo workspaces; for single repos, drop those sections and the Repo column from Component Overview.

**Component specs** — one numbered file per implementation session. Use the [component template](templates/component.md).
